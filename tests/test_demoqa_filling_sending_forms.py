...
def test_filling_sending_forms():
    ...
    browser.element('#dateOfBirthInput').should(have.value(datetime.now().strftime('%d %b %Y'))).click()
    browser.element('.react-datepicker__month-select').element('[value="1"]').should(have.exact_text('February')).click()
    browser.element('.react-datepicker__year-select').element('[value="1992"]').should(have.exact_text('1992')).click()
    browser.all('.react-datepicker__week')[4].all('[role="option"]')[6].should(have.exact_text('29')).click()
